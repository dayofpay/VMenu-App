...
const ShowCheckout = ({ objectData }) => {
  ...
  const handleNextStep = () => {
    ...
    if (currentStep === 1) {
      ...
      Object.keys(validationSettings).forEach((key) => {
        ...
        if (rules.required && !value) {
          newErrors[key] = Validations.Required_Field; // "Това поле е задължително!"
        } else if (rules.regex && value && !rules.regex.test(value)) {
          newErrors[key] = Validations.Invalid_Format; // "Невалиден формат!"
        }
      });
    ...
    // Get button text from translations
  ...
  const handlePreviousStep = () => {
    ...
    const buttonText = menuLanguage.Finalize_Order.Header.Back_Button; // "Назад"
    do_action("click_button", { button_name: buttonText });
};
      ...
      <header style={styles.header}>
        <div style={styles.headerContent}>
          <Link to="/cart" style={styles.backButton}>
            ...
            {menuLanguage.Finalize_Order.Header.Back_Button} {/* "Назад" */}
          </Link>
          <h1 style={styles.title}>
            {menuLanguage.Finalize_Order.Header.Text} {/* "Финализиране на поръчка" */}
          </h1>
